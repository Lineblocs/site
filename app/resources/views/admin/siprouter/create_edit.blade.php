...
@section('content')
    ...
    <ul class="nav nav-tabs">
        ...
        @if (!empty($servers))
            <li><a href="#tab-servers" data-toggle="tab"> {{ trans('admin/modal.servers') }}</a></li>
        @endif
        @if (!empty($rtpproxies))
            <li><a href="#tab-rtpproxies" data-toggle="tab"> {{ trans('admin/modal.rtpproxies') }}</a></li>
        @endif
    ...
    @if (isset($router))
        {!! Form::model($router, [
            'url' => url('admin/router') . '/' . $router->id,
            'method' => 'put',
            'class' => 'bf',
            'files' => true,
        ]) !!}
    @else
        {!! Form::open(['url' => url('admin/router'), 'method' => 'post', 'class' => 'bf', 'files' => true]) !!}
    @endif
    <!-- Tabs Content -->
    <div class="tab-content">
        ...
        <div class="tab-pane active" id="tab-general">
            <div class="form-group  {{ $errors->has('name') ? 'has-error' : '' }}">
                {!! Form::label('name', trans('admin/siprouters.name'), ['class' => 'control-label']) !!}
                <div class="controls">
            ...
            <div class="form-group  no-padding {{ $errors->has('ip_address') ? 'has-error' : '' }}">
                <div class="col-md-6">
                    {!! Form::label('ip_address', trans('admin/siprouters.ip_address'), ['class' => 'control-label']) !!}
                    <div class="controls">
                ...
                <div class="col-md-6">
                    {!! Form::label('ip_address_range', trans('admin/siprouters.ip_address_range'), ['class' => 'control-label']) !!}
                    <div class="controls">
                        {!! Form::select('ip_address_range', $ranges, null, ['class' => 'form-control']) !!}
            ...
            <div class="form-group no-padding {{ $errors->has('private_ip_address') ? 'has-error' : '' }}">
                <div class="col-md-6">
                    {!! Form::label('private_ip_address', trans('admin/siprouters.private_ip_address'), [
                        'class' => 'control-label',
                    ]) !!}
                    <div class="controls">
                        {!! Form::text('private_ip_address', null, ['class' => 'form-control']) !!}
                        <span class="help-block">{{ $errors->first('private_ip_address', ':message') }}</span>
                    </div>
                </div>
                <div class="col-md-6">
                    {!! Form::label('private_ip_address_range', trans('admin/siprouters.private_ip_address_range'), [
                        'class' => 'control-label',
                    ]) !!}
                    <div class="controls">
                        {!! Form::select('private_ip_address_range', $ranges, null, ['class' => 'form-control']) !!}
                        <span class="help-block">{{ $errors->first('private_ip_address_range', ':message') }}</span>
                    </div>
                </div>
            </div>
            <div class="form-group no-padding {{ $errors->has('private_ip_address') ? 'has-error' : '' }}">
                {!! Form::label('region', trans('admin/siprouters.region'), ['class' => 'control-label']) !!}
                <div class="controls">
            ...
            <div class="form-group  {{ $errors->has('active') ? 'has-error' : '' }}">
                {!! Form::label('active', trans('admin/siprouters.active'), ['class' => 'control-label']) !!}
                <div class="controls">
                            ...
                            <tbody>
                                @foreach ($servers as $server)
                                    <tr>
                                        ...
                                        <td>
                                            <button type="button" class="btn btn-danger del-server"
                                                data-id="{{ $server->id }}">Delete</button>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
