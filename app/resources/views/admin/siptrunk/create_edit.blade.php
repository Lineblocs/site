...
@section('content')
    ...
    <ul class="nav nav-tabs">
        ...
        @if (!empty($orig_settings))
            <li><a href="#tab-origination" data-toggle="tab"> {{ trans('admin/modal.origination') }}</a></li>
        @endif
        @if (!empty($term_settings))
            <li><a href="#tab-termination" data-toggle="tab"> {{ trans('admin/modal.termination') }}</a></li>
        @endif
        @if (!empty($trunk))
    ...
    @if (isset($trunk))
        {!! Form::model($trunk, [
            'url' => url('admin/trunk') . '/' . $trunk->id,
            'method' => 'put',
            'class' => 'bf',
            'files' => true,
        ]) !!}
    @else
        {!! Form::open(['url' => url('admin/trunk'), 'method' => 'post', 'class' => 'bf', 'files' => true]) !!}
    @endif
    <!-- Tabs Content -->
    <div class="tab-content">
        ...
        <div class="tab-pane active" id="tab-general">
            <div class="form-group  {{ $errors->has('name') ? 'has-error' : '' }}">
                {!! Form::label('name', trans('admin/siptrunks.name'), ['class' => 'control-label']) !!}
                <div class="controls">
            ...
            <div class="tab-pane active" id="tab-general">
                <div class="form-group  {{ $errors->has('workspace_id') ? 'has-error' : '' }}">
                    {!! Form::label('workspace_id', trans('admin/siptrunks.workspace_id'), ['class' => 'control-label']) !!}
                    <div class="controls">
                        {!! Form::select('workspace_id', $workspaces, !empty($trunk) ? $trunk->workspace_id : null, [
                            'class' => 'form-control',
                        ]) !!}
            ...
            <div class="form-group  {{ $errors->has('record') ? 'has-error' : '' }}">
                {!! Form::label('record', trans('admin/siptrunks.record'), ['class' => 'control-label']) !!}
                <div class="controls">
                    {!! Form::label('record', trans('admin/admin.yes'), ['class' => 'control-label']) !!}
                    {!! Form::radio('record', '1', !empty($trunk) ? $trunk->record : 'false') !!}
                    {!! Form::label('record', trans('admin/admin.no'), ['class' => 'control-label']) !!}
                    {!! Form::radio('record', '0', !empty($trunk) ? $trunk->record : 'true') !!}
                    <span class="help-block">{{ $errors->first('confirmed', ':message') }}</span>
                </div>
            </div>
            <div class="form-group  {{ $errors->has('active') ? 'has-error' : '' }}">
                {!! Form::label('active', trans('admin/siptrunks.active'), ['class' => 'control-label']) !!}
                <div class="controls">
                    {!! Form::label('active', trans('admin/admin.yes'), ['class' => 'control-label']) !!}
                    {!! Form::radio('active', '1', !empty($trunk) ? $trunk->active : 'false') !!}
                    {!! Form::label('active', trans('admin/admin.no'), ['class' => 'control-label']) !!}
                    {!! Form::radio('active', '0', !empty($trunk) ? $trunk->active : 'true') !!}
                    <span class="help-block">{{ $errors->first('confirmed', ':message') }}</span>
                </div>
            </div>
        </div>
