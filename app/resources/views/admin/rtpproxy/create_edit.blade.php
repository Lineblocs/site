...
@section('content')
    ...
    @if (isset($rtpproxy))
        {!! Form::model($rtpproxy, [
            'url' => url('admin/rtpproxy') . '/' . $rtpproxy->id,
            'method' => 'put',
            'class' => 'bf',
            'files' => true,
        ]) !!}
    @else
        {!! Form::open(['url' => url('admin/rtpproxy'), 'method' => 'post', 'class' => 'bf', 'files' => true]) !!}
    @endif
    <!-- Tabs Content -->
    <div class="tab-content cc">
        ...
        <div class="tab-pane active" id="tab-general">
            <div class="form-group  {{ $errors->has('rtpproxy_sock') ? 'has-error' : '' }}">
                {!! Form::label('rtpproxy_sock', trans('admin/rtpproxies.rtpproxy_sock'), ['class' => 'control-label']) !!}
                <div class="controls">
                    {!! Form::text('rtpproxy_sock', null, ['class' => 'form-control']) !!}
                    <span class="help-block">{{ $errors->first('rtpproxy_sock', ':message') }}</span>
                </div>
            </div>
            <div class="form-group  no-padding {{ $errors->has('set_id') ? 'has-error' : '' }}">
                <div class="col-md-6">
                    {!! Form::label('set_id', trans('admin/rtpproxies.set_id'), ['class' => 'control-label']) !!}
                    <div class="controls">
                ...
                <div class="col-md-6">
                    {!! Form::label('priority', trans('admin/rtpproxies.priority'), ['class' => 'control-label']) !!}
                    <div class="controls">
                        {!! Form::select('priority', $ranges, null, ['class' => 'form-control']) !!}
            ...
            <div class="form-group no-padding {{ $errors->has('private_ip_address') ? 'has-error' : '' }}">
                {!! Form::label('router_id', trans('admin/rtpproxies.router'), ['class' => 'control-label']) !!}
                <div class="controls">
                    {!! Form::select('router_id', $routers, null, ['class' => 'form-control']) !!}
